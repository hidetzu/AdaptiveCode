...
namespace ServiceDiscoveryClient
{
    class Program
    {
        ...
        {
            var foundEndpoints = FindEndPointsByContract<CalculatorService>();

            if (!foundEndpoints.Any())
            {
                System.Console.WriteLine("No endpoints were found.");
            }
            else
            {
    
                var binding = new BasicHttpBinding();
                var channelFactory = new ChannelFactory<CalculatorService>(binding);
